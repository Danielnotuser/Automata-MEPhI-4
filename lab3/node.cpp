...
std::any Operation::ex()
{
    ...
    switch (id)
    {
        ...
        case ';':       inner.refresh(op[0]); inner.refresh(op[1]); return 0;
        case '=':       if (op[0]->type >= 0) {Variable *v = std::any_cast<Variable*>(op[0]); v->set_var(Operand(op[1]).get_val());} else std::cerr << "Error! Um.. lvalue of assignment is not a variable..";
        case DEREF:     return deref(op[0]);
        case ADDR:      return address(op[0]);
        case '[':       return arr_el_ref(op[0], op[1]);
        case UMINUS:    return Operand(0, 0)  - Operand(op[0]);
        case '+':       return Operand(op[0]) + Operand(op[1]);
        case '-':       return Operand(op[0]) - Operand(op[1]);
        case '*':       return Operand(op[0]) * Operand(op[1]);
        case '/':       return Operand(op[0]) / Operand(op[1]);
        case '%':       return Operand(op[0]) + Operand(op[1]);
